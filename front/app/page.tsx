...
export default function Home() {
  ...
  return (
    <div>
      <h1 className="text-xl sm:text-2xl font-bold mb-6">Artigos</h1>
      ...
      <div className="flex flex-col sm:flex-row gap-3 mb-6">
        ...
        <input
          ...
          className="w-full sm:w-40 border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-400"
        />
      </div>
        ...
        <div className="space-y-4">
          {data?.items.map((item) => (
            <Link
              ...
              className="block bg-white rounded-xl border border-gray-200 p-4 sm:p-5 hover:border-indigo-300 hover:shadow-sm transition-all"
            >
      ...
      {totalPages > 1 && (
        <div className="flex justify-center gap-2 mt-8">
          <button
            ...
            className="px-4 py-2 min-h-[44px] rounded-lg border border-gray-300 text-sm disabled:opacity-40 hover:bg-gray-100 transition-colors"
          >
            Anterior
          </button>
          <span className="px-4 py-2 min-h-[44px] flex items-center text-sm text-gray-600">
            {page} / {totalPages}
          </span>
          <button
